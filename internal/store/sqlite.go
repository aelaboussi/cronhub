// Package store provides the v1 Store: SQLite via modernc.org/sqlite (pure Go,
// no cgo, so cross-compilation stays trivial). A Postgres impl for multi-node is
// a future phase satisfying the same ports.Store.
package store
...
CREATE TABLE IF NOT EXISTS runs (
	...
	started   INTEGER NOT NULL,   -- unix nanos
...
CREATE INDEX IF NOT EXISTS idx_runs_job ON runs(job_name, started DESC);`)
	...
	// Lightweight migration: add columns introduced after a DB may have been
	// created. ADD COLUMN is idempotent-guarded by checking existing columns.
	return s.ensureColumn("jobs", "notify", "TEXT NOT NULL DEFAULT 'log'")
}

// ensureColumn adds a column to a table if it is not already present. This lets
// databases created by an earlier cronhub version pick up new fields without a
// manual migration.
...
FROM runs WHERE job_name = ? ORDER BY started DESC LIMIT ?`, jobName, limit)
	...
	for rows.Next() {
		...
		r.Started = time.Unix(0, startedNs)
